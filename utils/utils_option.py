...
def find_last_checkpoint(save_dir, net_type='G'):
    ...
    Return:
        init_iter: iteration number (int)
        init_path: model path (str or None)
    """
    # 1. 使用 glob 查找所有匹配的文件
    # 注意：os.path.join 确保路径在不同操作系统上都能正确工作
    file_list = glob.glob(os.path.join(save_dir, '*_{}.pth'.format(net_type)))
    
    if file_list:
        iter_exist = []
        
        # 定义正则表达式，只提取文件名开头处的数字 (\d+)
        # 我们使用原始的 findall 逻辑，但要更安全地处理结果
        pattern = re.compile(r"(\d+)_{}.pth".format(net_type))
        
        for file_path in file_list:
            # 只取文件名部分，避免路径干扰
            file_name = os.path.basename(file_path)
            
            # 使用 re.match 或 re.findall 来提取数字
            iter_match = pattern.findall(file_name)
            
            # 关键修正：检查列表是否包含匹配项
            if iter_match:
                try:
                    # 确保提取到的字符串是有效的整数
                    iter_number = int(iter_match[0])
                    iter_exist.append(iter_number)
                except ValueError:
                    # 如果文件名中提取到的内容不是数字，则跳过
                    print(f"Warning: Skipping file '{file_name}' due to non-integer iteration label.")
                    continue
            else:
                # 遇到 'best_G.pth' 或其他不带数字的文件名时，跳过，不抛出异常
                print(f"Warning: Skipping file '{file_name}' as no iteration number was found.")
                continue
        ...
        if iter_exist:
            init_iter = max(iter_exist)
            init_path = os.path.join(save_dir, '{}_{}.pth'.format(init_iter, net_type))
            print(f"Resuming training from iteration {init_iter}.")
        else:
            # 如果目录中有文件，但都不是有效检查点
            init_iter = 0
            init_path = None
