...
def imread_uint(path, n_channels=1):
    """
    Read an image and return uint format.
    Args:
        path: image path
        n_channels: 1 for infrared, 3 for visible
    Returns:
        np.ndarray in shape (H, W, 3), dtype=uint8
    """
    if n_channels == 1:
        # ----- Infrared image -----
        img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)  # (H, W)
        img = np.expand_dims(img, axis=2)             # (H, W, 1)
        img = np.repeat(img, 3, axis=2)               # (H, W, 3) 灰度扩展为3通道
    elif n_channels == 3:
        # ----- Visible image -----
        ...
        y_channel = img_ycbcr[:, :, 0:1]              # 取出Y通道 (H, W, 1)
        img = np.repeat(y_channel, 3, axis=2)         # 扩展为 (H, W, 3)
    else:
