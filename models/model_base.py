...
class ModelBase():
    ...
    def update_E(self, decay=0.999):
        ...
        for k in netE_params:
            if k in netG_params:
                netE_params[k].data.mul_(decay).add_(netG_params[k].data, alpha=1 - decay)
            else:
                if self.opt['rank'] == 0:
                    print(f'[EMA Warning] Parameter {k} not found in netG, skipped.')

        # ----------- 同步 buffers (e.g., running_mean/var) -----------
        netG_buffers = dict(netG.named_buffers())
        netE_buffers = dict(netE.named_buffers())

        for k in netE_buffers:
            if k in netG_buffers:
                netE_buffers[k].data.copy_(netG_buffers[k].data)
